...
const handleClickOutside = (event: MouseEvent) => {
    ...
    if (isMenuOpen.value && nav && !nav.contains(event.target as Node)) {
        closeMenu();
    }
};
    ...
    <header class="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-slate-200 shadow-sm">
        <nav class="max-w-6xl mx-auto px-6 py-4">
            <div class="flex items-center justify-between">
                <RouterLink to="/" class="flex items-center gap-2 text-xl font-semibold text-slate-900" @click="closeMenu">
                    <span
                ...
                <!-- Hamburger Button -->
                <button class="md:hidden p-2 text-slate-600 hover:text-slate-900 focus:outline-none"
                    @click.stop="toggleMenu" aria-label="Toggle menu">
            ...
            <Transition name="slide-down">
                <div v-if="isMenuOpen" class="md:hidden mt-4 pb-4 border-t border-slate-100">
                    <div class="flex flex-col gap-4 pt-4">
                        <template v-if="isAuthenticated">
                            <div class="flex items-center gap-3 px-2">
                                ...
                                <div class="text-sm">
                                    <p class="font-semibold text-slate-900">{{ user?.displayName || user?.email }}</p>
