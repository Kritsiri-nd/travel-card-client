...
const AUTH_SERVER_BASE_URL =
  ...
  resolveAuthServerBase(API_BASE_URL);
...
const handleLogin = async () => {
  ...
  if (!isValidEmail(loginForm.value.email.trim())) {
    loginError.value = "รูปแบบอีเมลไม่ถูกต้อง";
    return;
  }
...
const handleRegister = async () => {
  if (
    ...
    registerError.value = "กรุณากรอกข้อมูลให้ครบทุกช่อง";
    return;
  }
  if (!isValidEmail(registerForm.value.email.trim())) {
    registerError.value = "รูปแบบอีเมลไม่ถูกต้อง";
    return;
  }
