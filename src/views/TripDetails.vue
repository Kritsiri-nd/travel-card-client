...
const fetchTrip = async () => {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : "Failed to load trip details";
  } finally {
    loading.value = false;
  }
};
  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-else-if="error" class="text-center py-12">
        ...
        <button @click="fetchTrip" class="mt-4 px-4 py-2 bg-cyan-500 text-white rounded-lg hover:bg-cyan-600">
          Retry
        </button>
      </div>
      ...
      <div v-else-if="trip">
        ...
        <div class="relative h-96 rounded-2xl overflow-hidden shadow-xl mb-8">
          <img :src="trip.photos?.[0] || 'https://via.placeholder.com/1200x600?text=No+Image'" :alt="trip.title" class="w-full h-full object-cover" />
          <div class="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex flex-col justify-end p-8">
            ...
            <div class="flex items-center text-white/90 space-x-4">
              ...
              <span class="flex items-center" v-if="trip.latitude && trip.longitude">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-8">
            <div class="bg-white rounded-2xl p-8 shadow-sm">
              ...
              <div class="mt-6 flex flex-wrap gap-2">
                 <span v-for="tag in trip.tags" :key="tag" class="px-3 py-1 bg-cyan-100 text-cyan-800 rounded-full text-sm font-medium">
                    #{{ tag }}
                 </span>
              </div>
            </div>
            ...
            <!-- Gallery -->
            <div class="bg-white rounded-2xl p-8 shadow-sm" v-if="trip.photos && trip.photos.length > 0">
              <h2 class="text-2xl font-bold text-gray-900 mb-4">Gallery</h2>
              <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                 <div v-for="(photo, index) in trip.photos" :key="index" class="aspect-square rounded-lg overflow-hidden cursor-pointer hover:opacity-90 transition-opacity">
          ...
          <div class="space-y-8">
            <div class="bg-white rounded-2xl p-6 shadow-sm sticky top-24">
              ...
              <div class="mt-6">
                  <a :href="getMapLink(trip.latitude, trip.longitude)" target="_blank" class="w-full bg-sky-500 hover:bg-sky-600 text-white font-semibold py-3 px-6 rounded-xl transition-colors duration-200 flex items-center justify-center">
                      Open in Google Maps
                  </a>
              </div>
            </div>
