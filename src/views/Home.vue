...
const filteredTrips = computed(() => {
    ...
    return trips.value.filter((trip) => {
        ...
        const matchTags = trip.tags?.some((tag) => tag.toLowerCase().includes(query)) || false;
        return matchTitle || matchDesc || matchTags;
    });
});

const emptyMessage = computed(() => `No trips found matching "${searchQuery.value}"`);
...
<template>
    <div class="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-cyan-200 selection:text-cyan-900">
        <section class="px-6 py-12 max-w-6xl mx-auto space-y-12">
            ...
            <div>
                ...
                <TransitionGroup tag="div" name="list" class="grid gap-8 md:grid-cols-2 lg:grid-cols-3 relative" v-else>
                    <TripCard v-for="trip in filteredTrips" :key="trip.id" :trip="trip"
                        :date-label="formatDate(trip.createdAt || trip.updatedAt)" :excerpt="truncate(trip.description)"
